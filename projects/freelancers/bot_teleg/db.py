...
def export_data_to_xlsx(get_user_func):
    ...
    user_ids, usernames, amounts, dates = [], [], [], []
    sheet_name = 'payments'

    for payment in get_all_payments_history():
        try:
            user_id, username, amount, date = payment['user_id'], f"https://t.me/{get_user_func(payment['user_id']).username}", payment['amount'], get_date_from_timestamp(payment['date'])
        except errors.exceptions.bad_request_400.PeerIdInvalid:
            user_id, username, amount, date = payment['user_id'], 'Неверный id', payment['amount'], get_date_from_timestamp(payment['date'])
        user_ids.append(user_id)
        usernames.append(username)
        amounts.append(amount)
        dates.append(date)

    pandas.DataFrame({'user_id': user_ids, 'username': usernames, 'amount': amounts, 'date': dates}).to_excel(writer, sheet_name=sheet_name)

    '''выгрузка подписчиков'''
    user_ids, usernames, sub_ids, dates = [], [], [], []
    sheet_name = 'subscribers'

    for sub in get_subscribers(type='all'):
        user_id = sub['user_id']
        sub_id = sub['sub_id']
        date = get_date_from_timestamp(sub['finish_date'])
        try:
            username = f"https://t.me/{get_user_func(sub['user_id']).username}"
        except errors.exceptions.bad_request_400.PeerIdInvalid:
            username = 'Неверный id'
    ...
    pandas.DataFrame({'user_id': user_ids, 'username': usernames, 'sub_id': sub_ids, 'date': dates}).to_excel(writer, sheet_name=sheet_name)
...
def set_sub_period_user(sub_id, finish_date, user_id):
    conn = get_conection()
    cursor = conn.cursor()

    sql = f"""
UPDATE subscribers SET finish_date = ? WHERE sub_id = ? AND user_id = ?
"""
    
    cursor.execute(sql, (finish_date, sub_id, user_id))
    conn.commit()
    conn.close()
...
def check_email_valid(email):
    req = requests.get(f"https://api.2ip.me/email.txt?email={email}").json()
    return req
