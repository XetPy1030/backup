...
async def some_inline_handler(inline_query: types.CallbackQuery):
    ...
    match inline_query.data.split(':')[1:]:
        ...
        case "train", :
            await message.answer(lang['train_description'], reply_markup=markups.inline_buy_train)
        case "train_buy", :
            payment_id, markup = qiwi.generate_check_markups()
            cost_train = config["Login"]["cost_train"]
            number = config["Login"]["number"]
            await message.answer(
                f'Переведите {cost_train}, на номер по qiwi {number}, с комментарием {payment_id}',
                reply_markup=markup
                )
        case "train_check", uid:
            pay_id = inline_query.data.split(':')[2]
            is_pay = qiwi.check_payment(pay_id)

            if not is_pay:
                await inline_query.answer('Оплата не прошла')
                return

            user = db.get_user(inline_query.from_user.id)
            user['is_buy_train'] = True
            db.set_user(inline_query.from_user.id, user)
            await message.answer('Успешно!')
        case _:
            await inline_query.answer('⚙️ Данный разрел находится в разработке')
