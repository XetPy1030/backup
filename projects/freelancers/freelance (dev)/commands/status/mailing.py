...
async def mailing(message: types.Message):
    ...
    match status:
        case "choice":
            status = reversed_buttons[message.text.split(' - ')[1]] \
                if message.text.startswith(lang['button']) else (
                    'train' if message.text == 'Обучение' else 'all')
